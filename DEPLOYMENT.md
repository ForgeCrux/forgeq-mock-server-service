...
| Region | `us-central1` |
| Service name | `forgeq-mock-server-service` |
...
```bash
docker build -t forgeq-mock-server-service:local .
docker run --rm -p 8080:8080 forgeq-mock-server-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/forgeq-mock-server-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy forgeq-mock-server-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
